...
import { RefreshCw, Droplets, Landmark, TrendingUp, TrendingDown, Activity, Gauge } from "lucide-react";
...
// 综合指数卡片：分位 + 趋势 + 解读
function IndexCard({ idx }: { idx: CompositeIndex }) {
  // 分位 → 颜色（越低越绿=宽松/冷，越高越红=紧/热）
  const zoneColor = idx.value > 70 ? "text-danger" : idx.value < 30 ? "text-success" : "text-muted-foreground";
  const zoneBg = idx.value > 70 ? "bg-danger/10" : idx.value < 30 ? "bg-success/10" : "bg-muted/20";
  const zoneLabel = idx.value > 70 ? "偏紧" : idx.value < 30 ? "宽松" : "中性";
  return (
    <GlassCard className="p-4">
      <div className="mb-1 flex items-baseline justify-between">
        <p className="text-xs font-medium text-muted-foreground">{idx.label}</p>
        <span className="text-[10px] text-muted-foreground/50">{idx.date}</span>
      </div>
      <div className="flex items-baseline gap-2">
      ...
      <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-muted/20">
        <div
          className={cn("h-full rounded-full transition-all", idx.value > 70 ? "bg-danger/60" : idx.value < 30 ? "bg-success/60" : "bg-primary/40")}
...
export function Liquidity() {
  const [data, setData] = useState<LiquidityData | null>(() => {
    try {
      const cached = storageGet(CACHE_KEY);
      return cached ? JSON.parse(cached) as LiquidityData : null;
    } catch { return null; }
  });
  ...
  const load = () => {
    ...
    setErr(false);
    api.liquidity()
      .then((d) => { setData(d); storageSet(CACHE_KEY, JSON.stringify(d)); })
      .catch(() => setErr(true))
  ...
  return (
    <div>
      <PageHeader
        title="资金供给"
        subtitle="国内外重要流动性指标 · 含历史趋势与美联储利率（数据仅供参考，不构成投资建议）"
        actions={
      ...
      {cnIndices.length > 0 && (
        <>
          <div className="mb-3 flex items-center gap-2">
            ...
            <span className="text-[11px] text-muted-foreground/50">多子指标加权合成 · 0-100 分位 · 越高=越紧/越热</span>
          </div>
          <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            {cnIndices.map((idx) => <IndexCard key={idx.label} idx={idx} />)}
          </div>
        </>
      )}
      ...
      <div className="mb-3 flex items-center gap-2">
        <h3 className="flex items-center gap-1.5 text-sm font-semibold text-muted-foreground">
          <Droplets className="h-4 w-4" /> 国内 · 杠杆资金
        </h3>
        <span className="text-[11px] text-muted-foreground/50">两融余额 / 融资净买入 · 东财，T+1 披露</span>
      ...
      <div className="mb-3 flex items-center gap-2">
        <h3 className="flex items-center gap-1.5 text-sm font-semibold text-muted-foreground">
          <Activity className="h-4 w-4" /> 国内 · 主力资金流
        </h3>
        <span className="text-[11px] text-muted-foreground/50">三大指数主力净流入 · 盘中实时（东财）</span>
      </div>
      ...
      {/* 国外（美国）· 美联储加息概率 */}
      {fedOdds && fedOdds.strikes.length > 0 && (
        <>
          <div className="mb-3 flex items-center gap-2">
            <h3 className="flex items-center gap-1.5 text-sm font-semibold text-muted-foreground">
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <GlassCard className="p-4">
          ...
          <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground/60">
            {us?.walcl ? `总资产 ${fmt(us.walcl.value, " 亿")}，较 1 年前 ${
              us.walcl.hist.length > 1
