...
import { RefreshCw, Loader2, Trash2, AlertCircle, X, ChevronsUpDown, ChevronUp, ChevronDown, LineChart, Pencil } from "lucide-react";
...
export function Portfolio() {
  ...
  const closeEdit = () => {
    setEditTarget(null); setEShares(""); setECost(""); setEBoughtDate("");
  };

  const saveEdit = async () => {
    if (savingEdit || !editTarget) return;
    const s = parseFloat(eShares), cc = parseFloat(eCost);
    if (!(s > 0) || !Number.isFinite(cc)) { setErr("数量须大于 0，成本价请填数字（可为负）"); return; }
    setSavingEdit(true); setErr(null);
    try {
      setData(await api.updateHolding(editTarget, s, cc, eBoughtDate || undefined));
      closeEdit();
    } catch (e) {
      ...
      {editTarget && createPortal(
        ...
        >
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="securities-edit-title"
            className="w-full max-w-md rounded-2xl border border-border/70 bg-background/95 p-5 shadow-2xl"
          >
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                <h3 id="securities-edit-title" className="text-base font-semibold">修改持仓</h3>
                <p className="mt-1 text-xs text-muted-foreground">
                  {data?.holdings.find((x) => x.code === editTarget)?.name || editTarget} ·{" "}
                  <span className="font-mono">{editTarget}</span>
                </p>
              </div>
              <button onClick={closeEdit} className="rounded-md p-1 text-muted-foreground hover:bg-muted/60 hover:text-foreground" title="关闭">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="space-y-3.5">
              <div className="grid grid-cols-3 gap-2.5">
                <label className="block">
                  <span className="mb-1 block text-xs text-muted-foreground">数量（股）</span>
                  <input value={eShares} onChange={(e) => setEShares(e.target.value.replace(/[^\d.]/g, ""))}
                         className="w-full rounded-xl border border-border bg-muted/20 px-2.5 py-2 text-sm font-mono outline-none focus:border-primary/60" />
                </label>
                <label className="block">
                  <span className="mb-1 block text-xs text-muted-foreground">成本价</span>
                  <input value={eCost} onChange={(e) => setECost(e.target.value.replace(/[^\d.-]/g, "").replace(/(?!^)-/g, ""))} placeholder="可为负"
                         className="w-full rounded-xl border border-border bg-muted/20 px-2.5 py-2 text-sm font-mono outline-none focus:border-primary/60" />
                </label>
                <label className="block">
                  ...
                  <input type="date" value={eBoughtDate} onChange={(e) => setEBoughtDate(e.target.value)}
                         title="年内买入按成本计本年盈亏，年前买入按年初价计；清空则按年前持有计"
              ...
              <div className="flex items-center justify-end gap-2 border-t border-border/40 pt-3">
                <button onClick={closeEdit} className="h-8 rounded-lg px-3 text-xs text-muted-foreground hover:bg-muted/50">取消</button>
...
/** 持仓明细行 + 可展开的择时信号详情行。 */
function SignalRows({ h, sig, open, onToggle, onRemove, onClose, onEdit }: {
  h: Holding;
      ...
      <tr className="border-b border-border/30">
        ...
        <td className="px-2 py-2.5">
          <div className="flex items-center justify-end gap-1">
            <button onClick={onEdit} className="rounded-lg p-1.5 text-muted-foreground transition hover:bg-black/20 hover:text-foreground" title="修改数量 / 成本">
