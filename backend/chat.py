...
SYSTEM_PROMPT = f"""你是 Market Workbench 里的投研助理。你可以调用工具获取客观数据来支撑回答，A 股工具一律传 6 位代码：
...
- 市场研判层：query_macro_composite（宏观总分与模块分解）/ query_liquidity_composite（中美流动性综合得分）/ query_sector_scores（行业评分与轮动位置）/ query_gold_score（黄金多维评分）/ query_bonds_curve（中债收益率曲线与利差）/ query_fedwatch（美联储利率追踪：ZQ 自算概率 + Polymarket 对比 + 边际变化）——这些是各页面的合成分与贡献分解，属客观数据口径；引用时陈述分值与状态即可，不要包装成买卖建议
...
_PUBLIC_MODE = bool(os.environ.get("MW_API_KEY", "").strip())  # 设了鉴权≈公网部署姿态
...
def _check_base_url(url: str) -> None:
    ...
    本地单用户（未设 MW_API_KEY）放行 127.0.0.1 等本机地址（方便接本机 Ollama / 网关），只挡 169.254 元数据；
    公网部署（设了 MW_API_KEY）额外禁内网，并解析域名核对，防 DNS 指向内网。"""
    p = urlparse(url or "")
